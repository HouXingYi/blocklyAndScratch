...
class Stage extends React.Component {
    constructor (props) {
        ...
        if (this.props.vm.renderer) {
            ...
            console.log('执行绘图 this.props.vm.renderer.draw();，然而postSpriteInfo才是最终绘图的api');
    ...
    updateRect () {
        console.log('更新rect 即画布的大小 rect');
    ...
    onMouseUp (e) {
        console.log('onMouseUp 抬起鼠标 准备停止拖动 StopDrag');
        const {x, y} = getEventXY(e);
        const mousePosition = [x - this.rect.left, y - this.rect.top];
        this.cancelMouseDownTimeout();
        this.setState({
        ...
        if (this.state.isDragging) {
            console.log('调用 onStopDrag，如果把这里注释，则对象背后的阴影不会消失，即没有进行draw的工作');
    ...
    onStartDrag (x, y) {
        console.log('开始拖拽！onStartDrag 获取当前拖拽的对象的id');
    ...
    onStopDrag (mouseX, mouseY) {
        console.log('停止拖拽！onStopDrag，获取拖拽对象的 x 和 y 坐标，准备绘图');
        ...
        if (this.props.useEditorDragStyle) {
            ...
            console.log('clearDragCanvas 清除临时拖动的对象，即背后有阴影的图像 aka this.dragCanvas');
